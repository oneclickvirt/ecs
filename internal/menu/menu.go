...
func PrintMenuOptions(preCheck utils.NetCheckResult, config *params.Config) {
	...
	fmt.Println("3. 精简版(系统信息+CPU+内存+磁盘+常用平台解锁+路由+测速节点5个)")
	fmt.Println("4. 精简网络版(系统信息+CPU+内存+磁盘+回程+路由+测速节点5个)")
	fmt.Println("5. 精简解锁版(系统信息+CPU+内存+磁盘IO+御三家+常用平台解锁+测速节点5个)")
	fmt.Println("6. 网络单项(IP质量检测+上游及三网回程+广州三网回程详细路由+全国延迟+TGDC+网站延迟+测速节点11个)")
	fmt.Println("7. 解锁单项(御三家解锁+常用平台解锁)")
	...
	fmt.Println("5. Unlock-Focused Test Suite (System Info + CPU + Memory + Disk IO + Basic Unlock Tests + Common Platform Unlock + 5 Speed Test Nodes)")
	fmt.Println("6. Network-Only Test (IP Quality Test + TGDC + Websites + 11 Speed Test Nodes)")
	fmt.Println("7. Unlock-Only Test (Basic Unlock Tests + Common Platform Unlock)")
