...
**结构化结果：** 顶层 schema 为 `goecs.report/v1`。`sections` 说明章节是否启用及章节级状态，`components` 使用各组件自己的 `schema_version` 和 `payload` 保存完整指标，`data_files` 记录数据快照版本/来源/数量/fallback，`tcp` 保存全部目标和握手样本，`text` 保存兼容文本。`ok` 表示所有已启用测试成功；任一已启用章节降级会使总状态至少为 `partial`；`unavailable`、`timeout`、`canceled`、`error`、`skipped` 分别表示无可用能力、超时、取消、执行错误和按条件跳过。使用有效参数 `-menu=false -json=-` 可抑制实时紧凑文本并向 stdout 写 JSON；保留默认菜单时，菜单本身仍会先写 stdout，Android/Termux 的启动期 DNS 检查也可能先输出平台诊断。`-json=<文件>` 以 `0600` 写文件；`-privacy` 会禁用分享、脱敏结构化身份字段，并将 JSON 的 `text` 置空。

**选项 1 的默认增强项：** 选择全测后会开启 Ping、TCP 握手探针、`deep`、多盘检测、CPU 20 秒压力测试和 IP 标签兼容显示；联网时再开启解锁、IP 质量、邮件、回程/路由、TGDC、网站延迟和测速。这里描述的是中文模式/语言规则归一前的全测 preset；英文模式随后会按下文规则关闭 Backtrace 和 NT3 中国省级路由，并换用国际 Ping/测速目标。明确命令行参数的优先级高于菜单预设。CPU 压力结果合并在 CPU 章节，不单独输出无意义的 OK 行。多盘只在发现的可写挂载点中创建有上限的临时普通文件，不写裸盘。

**硬件字段：** 增强系统报告包含 CPU 的逻辑线程/物理核/每核线程/插槽/cpuset，cgroup v1/v2 的 CPU quota/period/等效核数、内存 high/current/limit/swap 和 PID 上限，以及主板/BIOS、PCI 与驱动、GPU、NUMA、DIMM 容量/类型/速率、HugePages、物理盘、RAID 和控制器。Linux 网络调优字段包含当前/可用拥塞控制、默认 qdisc 和 TCP rmem/wmem。紧凑文本坚持一行一个含义，同一实体连续排列；TCP 加速方式、队列规则、接收缓冲和发送缓冲四行保持相邻。内存总量已在内存行显示，因此文本只显示 DIMM 数量，不再重复汇总 DIMM 容量；完整 DIMM 结构仍保留在 JSON。磁盘可在系统实际暴露时显示型号、固件、逻辑块、旋转/只读、NVMe/ATA 健康计数器和温度。字段受操作系统、权限、容器设备透传、驱动和硬件支持限制；读不到时是空值/不可用，不会推测伪造。DIMM 序列号不输出，`-privacy` 还会关闭分享并脱敏主机标识。

**CPU、内存和磁盘指标：** CPU 增强测试按 `GOMAXPROCS`、CPU affinity、cpuset 和 cgroup quota 取可用线程上限，显示请求/有效线程、时长、事件数和每秒事件数。测试期间会从 Linux sysfs 筛选 CPU 温度传感器，可用时显示基线、峰值和变化量；虚拟机或系统不暴露传感器时不显示温度。内存增强测试分别给出顺序读取、顺序写入、copy 带宽和随机依赖链延迟（ns），工作集与迭代次数有上限，不等同于内存规格的理论带宽；带宽越高越好，依赖链延迟越低越好。标准 fio 矩阵独立测量 4K Q1/Q32 随机读写和 1M Q1/Q8 顺序读写，显示 IOPS、带宽及 P50/P95/P99 完成延迟。Q1/Q32 表示单次测试的 I/O 队列深度；IOPS 是每秒完成的 I/O 次数。JSON 使用 `bandwidth_bytes_per_second`、`latency_p50_ns`、`latency_p95_ns` 和 `latency_p99_ns`，紧凑文本再动态换算为 KiB/MiB/GiB/s 与 us/ms/s；P50 是中位数，P95/P99 反映尾部慢请求，吞吐越高且延迟越低通常越好。`deep` 增加 ATTO 式 512B–64M 读写扫描；多盘矩阵对自动发现或 `-deep-disk-paths` 指定的挂载目录逐一执行。SMART self-test 和 GPU compute 具有修改设备状态或高负载的风险，因此即使开启 `deep` 也不会自动运行，必须显式提供 `-deep-smart-devices` 或 `-deep-gpu-device`。

**Ping 与 TCP：** ICMP Ping 保留发送/接收、丢包、Min/Avg/P50/P95/Max 和单次样本；紧凑文本优先显示 Avg/P95/丢包，完整值在 JSON 中保留。`-ping-sort=latency|name` 选择延迟或稳定名称顺序，`-ping-scope=auto|china|international` 选择三网或国际目标。TCP 每个目标默认尝试 3 次，保留成功/尝试、丢包、Min/Avg/P50/P95/Max 和失败样本；D/R/T/O 依次是 DNS 解析失败、连接被拒绝、超时和其他错误。文本使用一个表头、每个平台一行，不追加“全球/购物”等类别，也不会省略平台；汇总固定显示目标、握手、成功率和失败总数，只有发生失败时才追加非零失败原因。`-tcp-sort=name|latency` 选择稳定名称顺序或失败/丢包/高延迟优先；`-tcp-format=compact|full` 作为兼容参数保留，每次握手样本仍在 JSON 中。

**并发与显示顺序：** 基础信息先独立完成，随后依次完成 CPU、内存和磁盘等硬件性能测试。硬件阶段结束后，解锁、IP 质量、邮件、回程/路由、Ping、TGDC、网站延迟、NAT 和 TCP 等所有已启用的非测速单项同时开始；每个单项只写入自己的缓冲区。主输出仍按原有章节顺序消费结果：当前一个章节完成且它之前的章节已经显示时会立即输出，后续章节即使更早完成也只暂存，不会越序或与其他章节穿插。所有非测速单项完成并显示后才启动带宽测速，测速继续实时显示，从而避免探针与吞吐测试相互干扰。
...
**组件数据与错误隐私：** TCP 目标、省级路由、公共/私有测速及 transfer 节点、DNSBL、ASN 映射和媒体元数据分别归 `pingtest`、`nt3`、`speedtest`/`privatespeedtest`、`security`、`backtrace`、`UnlockTests` 管理，不由 `goecs` 集中硬编码上游私有源。各组件的 Go updater 由其自有定时/手动 Action 更新，高频测速数据每 6 小时，其余主要 registry 每日同步；抓取后进行解析、去重、稳定排序、schema/最小数量/数量突降校验，适用时还会探活。只有语义变化才提交；空数据、字段漂移、异常突降或探活失败不会覆盖最近有效快照。运行时优先加载组件远程最新快照，通过 manifest 的 schema、count、generated_at 和 SHA-256 校验后使用；CDN/Raw 不可用、数据无效或 `-data-offline` 时回退到组件编译期快照。经典 carrier 测速路径在同一次运行中只加载并验证一次私有 registry，后续运营商筛选复用该只读结果，远程失败不会按运营商重复扩大等待；结构化数据清单与结构化测速组件仍各自执行一次有边界的加载。结构化报告显示实际来源别名、数量、更新时间和 fallback；公开错误会清除远程 URL、查询参数、凭据样字和本地构建路径，仅保留稳定来源别名与错误分类。
...
常用新增参数：`-tcp`、`-tcp-format=compact|full`、`-tcp-sort=name|latency`、`-ping`、`-ping-sort=latency|name`、`-ping-scope=auto|china|international`、`-deep`、`-diskmc`、`-deep-disk-paths`、`-deep-smart-devices`、`-deep-burn-duration=20s`、`-deep-gpu-device`、`-hardware-budget`、`-timeout`、`-privacy`、`-data-offline` 和 `-json`。`-timeout` 默认且最大为 15 分钟；`-hardware-budget` 标准模式默认且最多 2 分钟，deep 模式最多使用全局截止时间；磁盘路径、SMART、CPU burn 和 GPU 子项只在 deep 下生效，burn 时长还会被硬件预算截断。`-ut-concurrency` 默认 20、最大 100。Go API 对应提供 `WithFullTestPreset`、`WithMaxDuration`、`WithHardwareBudget`、`WithDeepMode`、`WithDiskMultiCheck`、`WithDeepDiskPaths`、`WithDeepSMARTDevices`、`WithDeepBurnDuration`、`WithDeepGPUDevice`、`WithPrivacyMode`、`WithTCPProbe`、`WithTCPTextFormat`、`WithPingTest`、`WithPingSortOrder`、`WithPingScope`、`WithTCPSortOrder`、`WithJSONPath` 和 `WithDataOffline`。
...
如果 NVMe SSD的1M总吞吐量 < 1GB/s 表明存在严重的资源超开超售。
...
**Structured results:** The top-level schema is `goecs.report/v1`. `sections` records whether each chapter is enabled and its chapter-level status; `components` carries complete metrics in owner-specific `schema_version` payloads; `data_files` records snapshot version, source, count, and fallback; `tcp` retains every target and handshake sample; and `text` carries compatible text output. `ok` means all enabled work succeeded. Any degraded enabled section makes the aggregate status at least `partial`; `unavailable`, `timeout`, `canceled`, `error`, and `skipped` mean missing capability, deadline expiry, cancellation, execution failure, and conditional omission. Valid arguments `-menu=false -json=-` suppress compact live text and write JSON to stdout; with the default menu enabled, the menu itself still writes first, and Android/Termux startup DNS checks may also emit platform diagnostics. `-json=<file>` writes mode `0600`; `-privacy` disables sharing, redacts structured identities, and leaves JSON `text` empty.

**Option 1 defaults:** The full test enables Ping, the TCP handshake probe, `deep`, multi-disk discovery, a 20-second CPU pressure test, and IP-label compatibility. When connectivity is available it also enables unlock, IP quality, mail, backtrace/routing, TGDC, website latency, and speed tests. This describes the Chinese profile or the preset before language normalization; English mode subsequently disables the China-specific Backtrace and NT3 sections and selects international Ping/speed targets as described below. Explicit command-line settings override menu presets. CPU pressure results stay in the CPU chapter instead of producing a separate meaningless OK line. Multi-disk tests create only bounded temporary regular files on discovered writable mounts; they never write raw disks.

**Hardware fields:** The enhanced system report includes CPU logical threads, physical cores, threads per core, sockets and cpuset; cgroup v1/v2 CPU quota, period, effective core count, memory high/current/limit/swap, and PID limit; motherboard/BIOS; PCI devices and drivers; GPUs; NUMA; DIMM size, type, and speed; HugePages; physical disks; RAID arrays and controllers. Linux network tuning fields cover current and available congestion control, default qdisc, and TCP rmem/wmem. Compact text keeps one meaning per line and adjacent rows for the same entity; TCP acceleration, queue discipline, receive buffer, and send buffer are four consecutive rows. Because total memory already appears in the memory row, text shows the DIMM count without repeating an aggregate DIMM capacity; complete DIMM structures remain in JSON. Where the system exposes them, disks include model, firmware, logical block size, rotational/read-only flags, NVMe/ATA health counters, and temperature. Availability depends on the OS, permissions, container device passthrough, drivers, and hardware. Missing data remains empty or unavailable rather than being guessed. DIMM serials are not emitted; `-privacy` also disables sharing and redacts host identifiers.

**CPU, memory, and disk metrics:** The enhanced CPU test limits requested workers by `GOMAXPROCS`, CPU affinity, cpuset, and cgroup quota, then reports requested/effective threads, duration, events, and events per second. During the test it samples CPU sensors exposed through Linux sysfs and, when available, shows baseline, peak, and delta temperature; a VM or OS without exposed sensors simply has no temperature result. The enhanced memory test reports sequential read, sequential write, copy bandwidth, and random dependent-chain latency in nanoseconds. Its bounded working set is a measured workload, not a claim about theoretical DIMM bandwidth; higher bandwidth and lower dependent-chain latency are preferable. The standard fio matrix runs separate 4K Q1/Q32 random reads and writes and 1M Q1/Q8 sequential reads and writes, reporting IOPS, bandwidth, and P50/P95/P99 completion latency. Q1/Q32 is the I/O queue depth and IOPS is completed I/O operations per second. JSON uses `bandwidth_bytes_per_second`, `latency_p50_ns`, `latency_p95_ns`, and `latency_p99_ns`; compact text converts these dynamically to KiB/MiB/GiB/s and us/ms/s. P50 is the median while P95/P99 expose slow tail requests; normally higher throughput and lower latency are better. `deep` adds an ATTO-style 512B-64M read/write sweep. The multi-path matrix runs on auto-discovered mounts or directories explicitly supplied with `-deep-disk-paths`. SMART self-tests and GPU compute are not started merely by enabling `deep`; because they can change device state or create sustained load, they require `-deep-smart-devices` or `-deep-gpu-device` explicitly.

**Ping and TCP:** ICMP results retain sent/received counts, loss, Min/Avg/P50/P95/Max, and individual samples. Compact text emphasizes Avg/P95/loss while JSON keeps all values. `-ping-sort=latency|name` selects latency or stable-name order, and `-ping-scope=auto|china|international` selects carrier or international targets. TCP attempts each target three times by default and retains success/attempt counts, loss, Min/Avg/P50/P95/Max, and failed samples. D/R/T/O means DNS resolution failure, connection refused, timeout, and other errors. Text uses one header and one row per platform, omits category suffixes such as global/shopping, and never drops a platform. The summary always reports targets, handshakes, success rate, and total failures; only nonzero failure classes are appended when failures occur. `-tcp-sort=name|latency` selects stable name order or prioritizes failure, loss, and high latency. `-tcp-format=compact|full` remains a compatibility option, and JSON keeps every handshake sample.

**Concurrency and display order:** Basic information completes first, followed by the CPU, memory, disk, and other hardware performance stages in order. After hardware finishes, every enabled non-throughput section, including media, IP quality, mail, backtrace/routes, Ping, TGDC, website latency, NAT, and TCP, starts concurrently and writes only to its own buffer. The main output still consumes sections in the established order: a section is printed as soon as it and all preceding sections are ready, while an early later result remains buffered and can neither overtake nor interleave another section. Bandwidth speed tests start only after all non-throughput sections have completed and been displayed, and they retain real-time output so probes do not contaminate throughput measurements.
...
**Component data and error privacy:** TCP targets, province routes, public/private speed and transfer nodes, DNSBL zones, ASN mappings, and media metadata are owned by `pingtest`, `nt3`, `speedtest`/`privatespeedtest`, `security`, `backtrace`, and `UnlockTests`; `goecs` does not centrally hard-code private upstream definitions. Each component runs its Go updater from its own scheduled/manual Action: high-frequency speed data runs every six hours and the main registries run daily. Fetches are parsed, deduplicated, stable-sorted, and checked for schema, minimum count, and severe count drops, with health probes where applicable. Only semantic changes are committed; empty data, field drift, abnormal drops, or failed health checks cannot replace the last valid snapshot. Runtime prefers the component's latest remote snapshot after validating manifest schema, count, `generated_at`, and SHA-256. It falls back to the component's compile-time snapshot when CDN/Raw data is unavailable or invalid, or when `-data-offline` is set. During one run, the classic carrier path loads and validates its private registry once and reuses the read-only result for later carrier selection, so a remote failure is not repeated per carrier; the structured data inventory and structured speed component each perform their own bounded load. Structured reports show the actual stable source alias, count, update time, and fallback state. Public errors remove remote URLs, query strings, credential-like values, and local build paths, retaining only a stable source alias and error class.
...
Useful parameters are `-tcp`, `-tcp-format=compact|full`, `-tcp-sort=name|latency`, `-ping`, `-ping-sort=latency|name`, `-ping-scope=auto|china|international`, `-deep`, `-diskmc`, `-deep-disk-paths`, `-deep-smart-devices`, `-deep-burn-duration=20s`, `-deep-gpu-device`, `-hardware-budget`, `-timeout`, `-privacy`, `-data-offline`, and `-json`. `-timeout` defaults to and is capped at 15 minutes. `-hardware-budget` defaults to and is capped at 2 minutes in standard mode, while deep mode may use the global deadline. Disk paths, SMART, CPU burn, and GPU subtests take effect only with deep mode; burn duration is also clamped to the hardware budget. `-ut-concurrency` defaults to 20 and is capped at 100. Go callers can use `WithFullTestPreset`, `WithMaxDuration`, `WithHardwareBudget`, `WithDeepMode`, `WithDiskMultiCheck`, `WithDeepDiskPaths`, `WithDeepSMARTDevices`, `WithDeepBurnDuration`, `WithDeepGPUDevice`, `WithPrivacyMode`, `WithTCPProbe`, `WithTCPTextFormat`, `WithPingTest`, `WithPingSortOrder`, `WithPingScope`, `WithTCPSortOrder`, `WithJSONPath`, and `WithDataOffline`.
...
| Drive Type | 4K Bandwidth | 1M Bandwidth |
|------------|----------------------|----------------------|
| NVMe SSD | ≥ 200 MB/s | 5-10 GB/s |
...
1. **Primary Check**: 4K read and write bandwidth
...
2. **Secondary Check**: 1M total throughput
...
If an NVMe SSD's 1M total throughput is below 1 GB/s, it may indicate serious resource overselling.
...
**構造化結果:** 最上位schemaは `goecs.report/v1` です。`sections` は各章の有効状態と章単位の状態、`components` は所有コンポーネント固有の `schema_version` と完全な `payload`、`data_files` はスナップショットのバージョン/ソース/件数/fallback、`tcp` は全対象とハンドシェイクサンプル、`text` は互換テキストを保持します。`ok` は有効な全テストの成功を表し、いずれかの有効な章が劣化すると全体は少なくとも `partial` になります。`unavailable`、`timeout`、`canceled`、`error`、`skipped` は能力不足、期限切れ、キャンセル、実行エラー、条件による省略です。有効な引数 `-menu=false -json=-` はコンパクトなリアルタイムテキストを抑止してstdoutへJSONを出力します。デフォルトメニューを有効にした場合はメニューが先に出力され、Android/Termuxの起動時DNSチェックもプラットフォーム診断を出力する場合があります。`-json=<file>` は `0600` で保存し、`-privacy` は共有を無効にして構造化IDをマスクし、JSONの `text` を空にします。

**オプション1のデフォルト:** 全テストはPing、TCPハンドシェイク、`deep`、複数ディスク検出、20秒のCPU負荷テスト、IPラベル互換表示を有効にします。ネットワーク利用時はロック解除、IP品質、メール、バックトレース/ルーティング、TGDC、Web遅延、速度テストも有効にします。これは中国語プロファイルまたは言語正規化前のpresetの説明です。英語モードでは後述のとおり中国専用のBacktrace/NT3を無効にし、国際Ping/速度対象へ切り替えます。明示的なCLI値はメニューのプリセットより優先されます。CPU負荷結果はCPU章に統合され、単独の無意味なOK行を出しません。複数ディスクテストは発見された書き込み可能なマウント上の上限付き一時通常ファイルだけを使い、ローディスクには書き込みません。

**ハードウェアフィールド:** 拡張システムレポートにはCPUの論理スレッド、物理コア、1コア当たりのスレッド、ソケット、cpuset、cgroup v1/v2のCPU quota/period/実効コア数、メモリhigh/current/limit/swap、PID上限、マザーボード/BIOS、PCIデバイスとドライバ、GPU、NUMA、DIMMの容量/種類/速度、HugePages、物理ディスク、RAIDアレイとコントローラが含まれます。Linuxでは現在/利用可能な輻輳制御、デフォルトqdisc、TCP rmem/wmemも報告します。コンパクトテキストは1行1項目とし、同じ対象の行を連続配置します。TCP高速化方式、キュー規則、受信バッファ、送信バッファの4行も連続します。総メモリはすでにメモリ行にあるため、テキストではDIMM数だけを表示し、DIMM容量合計を重複表示しません。完全なDIMM構造はJSONに残ります。システムが公開している場合、ディスクのモデル、ファームウェア、論理ブロックサイズ、回転/読み取り専用フラグ、NVMe/ATAヘルスカウンタ、温度も表示します。取得可否はOS、権限、コンテナのデバイスパススルー、ドライバ、ハードウェアに依存します。欠損値は推測せず空または利用不可とします。DIMMシリアルは出力せず、`-privacy` は結果共有も無効にし、ホスト識別子をマスクします。

**CPU、メモリ、ディスク指標:** 拡張CPUテストは要求ワーカ数を `GOMAXPROCS`、CPU affinity、cpuset、cgroup quotaで制限し、要求/実効スレッド、時間、イベント数、秒間イベント数を報告します。テスト中にLinux sysfsのCPUセンサーをサンプリングし、利用可能な場合は開始、最高、変化温度を示します。センサーが露出されないVM/OSでは温度結果はありません。拡張メモリテストはシーケンシャル読み取り、書き込み、copy帯域、ランダム依存チェーン遅延（ns）を個別に報告します。上限付きワーキングセットの測定値であり、DIMMの理論帯域ではありません。帯域は高いほど、依存チェーン遅延は低いほど良好です。標準fioマトリクスは4K Q1/Q32ランダム読み/書きと1M Q1/Q8シーケンシャル読み/書きを分離し、IOPS、帯域、P50/P95/P99完了遅延を示します。Q1/Q32はI/Oキュー深度、IOPSは1秒当たりの完了I/O数です。JSONは `bandwidth_bytes_per_second`、`latency_p50_ns`、`latency_p95_ns`、`latency_p99_ns` を使用し、コンパクトテキストはKiB/MiB/GiB/sとus/ms/sへ動的に変換します。P50は中央値、P95/P99は遅い末尾要求を表し、通常は高いスループットと低い遅延が良好です。`deep` はATTO式512B–64M読み/書きスイープを追加します。複数パスマトリクスは自動検出マウントまたは `-deep-disk-paths` で明示したディレクトリ上で実行します。SMART self-testとGPU computeは `deep` だけでは開始せず、デバイス状態の変更や継続負荷の可能性があるため `-deep-smart-devices` または `-deep-gpu-device` が必要です。

**PingとTCP:** ICMP結果は送信/受信数、損失、Min/Avg/P50/P95/Max、個別サンプルを保持します。コンパクトテキストはAvg/P95/損失を中心に表示し、JSONは全値を保持します。`-ping-sort=latency|name` は遅延順または安定した名前順、`-ping-scope=auto|china|international` は三大キャリアまたは国際対象を選択します。TCPはデフォルトで各対象を3回試行し、成功/試行、損失、Min/Avg/P50/P95/Max、失敗サンプルを保持します。D/R/T/OはDNS解決失敗、接続拒否、タイムアウト、その他のエラーです。テキストは1つのヘッダーと1プラットフォーム1行で表示し、「global/shopping」などのカテゴリ接尾辞を付けず、プラットフォームを省略しません。集計は対象数、ハンドシェイク数、成功率、失敗総数を常に表示し、失敗時だけ0でない原因を追加します。`-tcp-sort=name|latency` は名前順、または失敗/損失/高遅延優先を選びます。`-tcp-format=compact|full` は互換オプションとして残り、JSONは全ハンドシェイクサンプルを保持します。

**並行実行と表示順:** 基本情報を最初に完了し、その後CPU、メモリ、ディスクなどのハードウェア性能テストを順番に完了します。ハードウェア完了後、メディア、IP品質、メール、バックトレース/経路、Ping、TGDC、Web遅延、NAT、TCPなど、有効な速度測定以外の項目を同時に開始し、各項目は専用バッファだけに書き込みます。メイン出力は従来の章順を維持し、対象とその前の章が準備できた時点ですぐ表示します。後の章が先に完了してもバッファで待機し、追い越しや出力混在は起きません。帯域速度テストは全ての非速度項目の完了・表示後にだけ開始し、リアルタイム表示を維持するため、他のプローブがスループット測定に干渉しません。
...
**コンポーネントデータとエラーのプライバシー:** TCP対象、省級ルート、公開/非公開の速度・transferノード、DNSBL zone、ASNマッピング、メディアメタデータは `pingtest`、`nt3`、`speedtest`/`privatespeedtest`、`security`、`backtrace`、`UnlockTests` が個別に管理し、`goecs` に非公開upstream定義を集中ハードコードしません。各コンポーネントのGo updaterは自身の定期/手動Actionから実行され、高頻度の速度データは6時間ごと、主要registryは毎日同期します。取得後にparse、重複排除、安定sort、schema/最小件数/大幅減少を検証し、適用可能なデータはhealth probeも行います。意味上の変更だけをcommitし、空データ、フィールドdrift、異常減少、health失敗は直近の有効snapshotを置換しません。実行時はmanifestのschema、count、`generated_at`、SHA-256を検証した最新リモートスナップショットを優先します。CDN/Rawが利用不可、データが不正、または `-data-offline` の場合はコンパイル時スナップショットにフォールバックします。1回の実行では従来のcarrier経路が非公開registryを一度だけロード・検証し、その後のcarrier選択で読み取り専用結果を再利用するため、リモート障害をcarrierごとに繰り返しません。構造化データ一覧と構造化速度コンポーネントは、それぞれ独立した上限付きロードを行います。構造化レポートは安定した実際のソース別名、件数、更新時刻、fallbackを示します。公開エラーからはリモートURL、クエリ、認証情報に似た値、ローカルビルドパスを削除し、安定ソース別名とエラー分類だけを残します。
...
主なパラメータは `-tcp`、`-tcp-format=compact|full`、`-tcp-sort=name|latency`、`-ping`、`-ping-sort=latency|name`、`-ping-scope=auto|china|international`、`-deep`、`-diskmc`、`-deep-disk-paths`、`-deep-smart-devices`、`-deep-burn-duration=20s`、`-deep-gpu-device`、`-hardware-budget`、`-timeout`、`-privacy`、`-data-offline`、`-json` です。`-timeout` はデフォルトかつ最大15分です。`-hardware-budget` は標準モードでデフォルトかつ最大2分、deepモードでは全体期限まで指定できます。ディスクパス、SMART、CPU burn、GPUのサブテストはdeep時だけ有効で、burn時間はハードウェア予算にも制限されます。`-ut-concurrency` はデフォルト20、最大100です。Go APIでは `WithFullTestPreset`、`WithMaxDuration`、`WithHardwareBudget`、`WithDeepMode`、`WithDiskMultiCheck`、`WithDeepDiskPaths`、`WithDeepSMARTDevices`、`WithDeepBurnDuration`、`WithDeepGPUDevice`、`WithPrivacyMode`、`WithTCPProbe`、`WithTCPTextFormat`、`WithPingTest`、`WithPingSortOrder`、`WithPingScope`、`WithTCPSortOrder`、`WithJSONPath`、`WithDataOffline` を利用できます。CLIの表示言語は `-lang=zh|en` のみで、日本語はこの利用説明の翻訳であり `-lang=ja` は提供していません。
...
NVMe SSDの1M総スループットが1GB/s未満の場合、深刻なリソースオーバーセリングが存在する可能性を示します。
