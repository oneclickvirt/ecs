...
- [TCP Handshake Latency Testing](#TCP-Handshake-Latency-Testing)
...
- [メールポート検出](#メールポート検出)
- [PING検出](#PING検出)
- [TCPハンドシェイク遅延テスト](#TCPハンドシェイク遅延テスト)
- [近隣スピードテスト](#近隣スピードテスト)
...
本项除原有的系统、CPU、内存、磁盘和网络信息外，还会尽可能显示 CPU 拓扑与容器限制、主板与 BIOS、PCI 与 GPU、NUMA 与 DIMM、HugePages、物理盘与 RAID，以及磁盘健康和温度。这些信息可用于判断虚拟机实际可用资源、硬件透传和存储健康；受系统、权限或虚拟化限制而无法读取的字段不会猜测显示。
...
NAT行为: 在环境支持时进一步显示端口是否保持、能否通过公网映射回连本机，以及地址映射和入站过滤方式，可辅助判断 P2P、语音视频和内网穿透的可用性。

TCP加速/队列：在同一行用 `/` 分隔，前项是 `cubic`、`bbr` 等拥塞控制算法，后项是 `fq`、`fq_codel` 等队列规则。代理或高带宽用途可结合两项判断网络调优情况。

TCP接收/发送缓冲：接收和发送仍各占一行，每行依次显示最小值、默认值和最大值，可用于排查高延迟或高带宽链路上的吞吐限制。

NUMA/DIMM：在同一行显示 NUMA 节点数和 DIMM 数量。虚拟机没有透传 DMI 信息时，DIMM 数量可能为 0，不代表没有内存。

HugePages：在同一行显示大页总数、空闲数和单页大小，用于判断大页内存是否配置及当前余量。

物理盘：每块磁盘用一行显示协议、健康状态和可用时的温度。`unsupported` 表示当前硬件、驱动、权限或虚拟化环境没有提供健康数据，不代表磁盘已经故障。
...
选项1会在原有 CPU 跑分中加入 20 秒压力测试。压力测试会让每个有效线程持续完成固定上限的素数校验，直到时间结束；有效线程数会结合 `GOMAXPROCS`、CPU affinity、cpuset 和 cgroup 配额确定，避免把不可用线程计入成绩。

`压力测试 : 20.005s / 2 线程 / 44.59 次/秒 / 892 次` 从左到右分别表示实际运行时间、有效工作线程数、全部线程合计每秒完成的事件数和事件总数。实际时间可能因工作线程完成最后一轮计算而略高于设定值；每完成一轮固定素数校验记为 1 次。比较机器时应使用相同版本、时长和线程条件，每秒事件数越高通常代表持续计算能力越强。系统提供温度传感器时还会显示测试前温度、峰值和变化量。
...
测试会分别给出顺序读取、顺序写入、内存复制带宽和随机访问延迟。前三项数值越高越好，随机访问延迟越低越好；这些结果反映当前机器在测试负载下的实际表现，不等同于内存条标称的理论带宽。
...
`fio` 会分别测试 4K Q1/Q32 随机读写和 1M Q1/Q8 顺序读写，并显示 IOPS、带宽及 P50/P95/P99 延迟。Q 表示队列深度，P50 是中位延迟，P95/P99 用来观察少量较慢的请求。选项1默认检测多个可写挂载点并执行更完整的块大小测试，只使用有大小上限的临时普通文件，不会写入裸盘。
...
每个数据源会独立保留正常、限流、需要认证、超时或不可用状态，某个来源暂时失败不会被当作“IP 干净”。DNS 黑名单检测也会区分未列入、已列入、其他标记、超时和查询错误，建议结合具体来源和多次结果判断。
...
结果会分开显示本机是否监听邮件端口、能否连接外部 SMTP 25 端口、目标域名按优先级解析出的 MX 服务器是否可达，以及各常用邮件协议端口是否可达。单项成功只代表对应方向和端点可用，不能用一个“25 端口开放”代替全部收发信能力。
...
省级网络目标覆盖 31 个省级区域及电信、联通、移动三家运营商，并按环境测试 IPv4、IPv6 或双栈。常规测试采用较短采样；选项1的深度测试会增加采样并补充更完整的省级路由，适合查看不同地区和运营商是否绕路。
...
结果会显示发送/接收次数、丢包率和 Min/Avg/P50/P95/Max 延迟。`-ping-sort=latency|name` 可选择按延迟或名称排序，`-ping-scope=auto|china|international` 可选择国内三网或国际目标；英文模式固定使用国际目标。
...
TCP 握手测试只执行 DNS 解析和 TCP 建连，不下载网页内容，也不等同于平台解锁测试。它比 ICMP Ping 更接近实际访问的连接阶段：测试目标会自动更新，无法取得最新有效列表时使用程序内置列表；同一主机和端口只测试一次。
...
In addition to the existing system, CPU, memory, disk, and network summary, this section shows CPU topology and container limits, motherboard and BIOS details, PCI devices and GPUs, NUMA and DIMM information, HugePages, physical disks and RAID, and disk health or temperature when available. These fields help identify the resources a VM can actually use, hardware passthrough, and storage health. Values hidden by the OS, permissions, or virtualization layer are left unavailable rather than guessed.
...
**NAT Behavior**: When supported, the report also shows port preservation, hairpin connectivity, address mapping behavior, and inbound filtering behavior. These results are useful for P2P, voice/video calls, and NAT traversal.

**TCP Acceleration/Queue**: One row separates the two values with `/`. The first is the congestion-control algorithm, such as `cubic` or `bbr`; the second is the queue discipline, such as `fq` or `fq_codel`.

**TCP Receive/Send Buffers**: Receive and send remain separate rows. Each shows the minimum, default, and maximum value and can help explain throughput limits on high-latency or high-bandwidth links.

**NUMA/DIMM**: One row shows the NUMA node count followed by the detected DIMM count. A VM without DMI passthrough may report zero DIMMs even though memory is available.

**HugePages**: One row shows the total count, free count, and page size, indicating whether huge pages are configured and how many remain free.

**Physical Disks**: Each disk uses one row for protocol, health, and temperature when available. `unsupported` means the current hardware, driver, permissions, or virtualization layer did not expose health data; it does not mean the disk has failed.
...
Option 1 adds a 20-second pressure test to the existing CPU benchmark. Every effective worker repeatedly completes the same bounded prime-number verification until time expires. The worker count respects `GOMAXPROCS`, CPU affinity, cpuset, and cgroup quota, so unavailable threads are not included.

`Pressure Test : 20.005s / 2 threads / 44.59 events/s / 892 events` means actual elapsed time, effective worker count, total events completed per second by all workers, and total events, in that order. The elapsed time can exceed the requested duration slightly while workers finish their current calculation. One completed prime-verification round counts as one event. Compare results only with the same version, duration, and thread conditions; higher events per second normally indicate stronger sustained compute performance. Temperature baseline, peak, and change are shown when a sensor is available.
...
The test reports sequential read, sequential write, memory-copy bandwidth, and random-access latency separately. Higher bandwidth is better, while lower random-access latency is better. These are measurements under the current workload, not the DIMM's advertised theoretical bandwidth.
...
`fio` runs separate 4K Q1/Q32 random read/write and 1M Q1/Q8 sequential read/write tests, reporting IOPS, bandwidth, and P50/P95/P99 latency. Q is queue depth, P50 is median latency, and P95/P99 expose slower tail requests. Option 1 checks multiple writable mounts and runs a broader block-size test. It uses size-bounded temporary regular files and never writes a raw disk.
...
Each source keeps its own available, rate-limited, authentication-required, timed-out, or unavailable state, so a temporary provider failure is not treated as a clean IP. DNS blacklist checks likewise distinguish clean, listed, other marked responses, timeouts, and query errors. Compare specific sources and repeat results before drawing a conclusion.
...
The result separates local mail listeners, outbound SMTP port 25, dynamically resolved MX servers tested in priority order, and the common mail protocol endpoints. A pass for one direction or endpoint does not mean that every send and receive path is available.
...
Results include sent/received counts, packet loss, and Min/Avg/P50/P95/Max latency. `-ping-sort=latency|name` selects latency or stable-name order, while `-ping-scope=auto|china|international` selects the target family. English output always uses international targets.
...
### TCP Handshake Latency Testing

Dependency project: [https://github.com/oneclickvirt/pingtest](https://github.com/oneclickvirt/pingtest)

This test performs DNS resolution and establishes a TCP connection only. It does not download page content and is not an unlock test. Targets are refreshed automatically; the valid list bundled with the program is used when the latest list cannot be obtained, and duplicate host/port pairs are tested only once.
...
従来のシステム、CPU、メモリ、ディスク、ネットワーク情報に加え、CPUトポロジとコンテナ制限、マザーボードとBIOS、PCIとGPU、NUMAとDIMM、HugePages、物理ディスクとRAID、取得可能な場合はディスクの健康状態と温度も表示します。VMが実際に利用できる資源、ハードウェアのパススルー、ストレージ状態の確認に役立ちます。OS、権限、仮想化によって取得できない値は推測せず、利用不可として扱います。
...
**NAT動作**: 対応環境では、ポート保持、ヘアピン接続、アドレスマッピング方式、受信フィルタリング方式も表示します。P2P、音声・映像通信、NAT越えの可否を判断する際に役立ちます。

**TCP高速化/キュー**: 1行に `/` 区切りで表示します。前は `cubic`、`bbr` などの輻輳制御アルゴリズム、後ろは `fq`、`fq_codel` などのキュー規則です。

**TCP受信/送信バッファ**: 受信と送信は別々の行に残し、それぞれ最小値、デフォルト値、最大値を表示します。高遅延または広帯域回線のスループット制限を確認できます。

**NUMA/DIMM**: 1行にNUMAノード数と検出したDIMM数を表示します。DMI情報が透過されないVMでは、メモリが存在してもDIMM数が0になる場合があります。

**HugePages**: 1行に総数、空き数、1ページのサイズを表示し、大ページの設定と空き容量を確認できます。

**物理ディスク**: 各ディスクについて、プロトコル、健康状態、取得可能な場合は温度を1行に表示します。`unsupported` は現在のハードウェア、ドライバ、権限、仮想化環境が健康情報を提供していないことを表し、故障を意味しません。
...
オプション1では、従来のCPUベンチマークに20秒の負荷テストを追加します。各実効ワーカーは時間終了まで同じ上限付き素数検証を繰り返します。実効スレッド数は `GOMAXPROCS`、CPU affinity、cpuset、cgroup quotaを考慮するため、利用できないスレッドは含まれません。

`負荷テスト : 20.005s / 2 threads / 44.59 events/s / 892 events` は、実際の経過時間、実効ワーカー数、全ワーカーが1秒に完了したイベント数、イベント総数の順です。ワーカーが実行中の計算を完了するため、実時間が指定時間をわずかに超える場合があります。固定の素数検証を1回完了すると1イベントです。同じバージョン、時間、スレッド条件で比較し、通常はevents/sが高いほど持続計算性能が高いと判断できます。センサーが利用可能な場合は開始温度、最高温度、変化量も表示します。
...
シーケンシャル読み取り、シーケンシャル書き込み、メモリコピー帯域、ランダムアクセス遅延を個別に表示します。帯域は高いほど、ランダムアクセス遅延は低いほど良好です。これは現在の負荷での実測値であり、DIMMの公称理論帯域とは異なります。
...
`fio` は4K Q1/Q32のランダム読み書きと1M Q1/Q8のシーケンシャル読み書きを個別に実行し、IOPS、帯域、P50/P95/P99遅延を表示します。Qはキュー深度、P50は中央値、P95/P99は一部の遅い要求を確認する指標です。オプション1では複数の書き込み可能なマウントを検出し、より広いブロックサイズを測定します。上限付きの一時通常ファイルだけを使用し、ローディスクには書き込みません。
...
各データソースは、正常、レート制限、認証要求、タイムアウト、利用不可を個別に保持するため、一時的な取得失敗を「クリーン」とは扱いません。DNSブラックリストも、未登録、登録済み、その他のマーク、タイムアウト、照会エラーを区別します。結論を出す際は、具体的なソースと複数回の結果を比較してください。
...
ローカルのメールポート待受、外部SMTP port 25への接続、優先度順に解決したMXサーバーへの接続、一般的なメールプロトコルの各エンドポイントを分けて表示します。一方向または一つのエンドポイントの成功だけでは、すべての送受信経路が利用可能とは限りません。
...
送信/受信回数、パケット損失、Min/Avg/P50/P95/Max遅延を表示します。`-ping-sort=latency|name` で遅延順または名前順、`-ping-scope=auto|china|international` で国内三大キャリアまたは国際対象を選択できます。英語出力では常に国際対象を使用します。
...
このテストはDNS解決とTCP接続確立だけを行い、ページ内容はダウンロードせず、ロック解除テストでもありません。対象は自動更新され、最新の有効リストを取得できない場合はプログラム同梱のリストを使用します。同じホストとポートは1回だけ測定します。
