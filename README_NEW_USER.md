...
TCP队列与缓冲：显示当前队列规则以及接收、发送缓冲范围。它们与拥塞控制方式连续展示，可用于排查高延迟或高带宽链路上的吞吐限制。
...
选项1会在原有 CPU 跑分中加入 20 秒压力测试。实际线程数会结合 CPU affinity、cpuset 和 cgroup 配额确定，避免把不可用线程计入成绩；结果中的每秒事件数越高通常代表计算性能越强。系统提供温度传感器时还会显示测试前温度、峰值和变化量，不提供传感器时则不显示温度。
...
TCP 握手测试衡量到各平台服务端建立连接所需的时间，比 ICMP Ping 更接近实际访问情况。每个平台默认尝试 3 次，逐行显示成功/尝试、丢包和 Min/Avg/P50/P95/Max；`D/R/T/O` 分别代表 DNS 解析失败、连接被拒绝、超时和其他错误。选项1默认启用本项，`-tcp-sort=name|latency` 可选择按平台名称或延迟排序。
...
**TCP Queue and Buffers**: Shows the active queue discipline and the receive/send buffer ranges. They are displayed with congestion control and can help explain throughput limits on high-latency or high-bandwidth links.
...
Option 1 adds a 20-second pressure test to the existing CPU benchmark. The effective worker count respects CPU affinity, cpuset, and cgroup quota so unavailable threads are not included in the score. Higher events per second generally indicate stronger compute performance. If the system exposes a CPU temperature sensor, the report also shows the starting temperature, peak, and change during the test.
...
This test measures the time required to establish a TCP connection to each platform, which is closer to real service access than ICMP Ping. Each platform is attempted three times by default and shown on its own row with success/attempt count, loss, and Min/Avg/P50/P95/Max. `D/R/T/O` means DNS failure, connection refused, timeout, and other error. Option 1 enables this test; `-tcp-sort=name|latency` selects platform-name or latency order.
...
**TCPキューとバッファ**: 現在のキュー規則と受信・送信バッファ範囲を表示します。輻輳制御方式と合わせて、高遅延または広帯域回線のスループット制限を確認できます。
...
オプション1では、従来のCPUベンチマークに20秒の負荷テストを追加します。実効スレッド数はCPU affinity、cpuset、cgroup quotaを考慮するため、利用できないスレッドはスコアに含まれません。通常は秒間イベント数が高いほど計算性能が高いと判断できます。CPU温度センサーを取得できる場合は、開始温度、最高温度、変化量も表示します。
...
各プラットフォームへのTCP接続確立時間を測定し、ICMP Pingより実際のサービス接続に近い遅延を確認します。デフォルトでは各プラットフォームを3回試行し、成功/試行、損失、Min/Avg/P50/P95/Maxを1行ずつ表示します。`D/R/T/O` はDNS失敗、接続拒否、タイムアウト、その他のエラーです。オプション1ではデフォルトで有効になり、`-tcp-sort=name|latency` でプラットフォーム名順または遅延順を選択できます。
